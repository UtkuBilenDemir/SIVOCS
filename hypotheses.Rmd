...
likert_plot(items_likert, h=FALSE, c=c(1,3,5)) + theme( axis.text.y = element_text( 
  size=g_tick_size_main), axis.text.x = element_text(
    size=g_tick_size_side), axis.title=element_text(size=g_tick_size_side), legend.text=element_text(size=g_tick_size_side)) + annotate("label", x = 3, y = -100, label = "bold(n) == 352/361", parse = TRUE) +
...
```{r, echo=FALSE, fig.width=10, fig.height=3, dev='svg', fig.retina=2, fig.align='center' }
...
```
```{r D2-barplot, echo=FALSE, fig.width=6, dev='svg', fig.retina=2, fig.align='center'}
...
ggplot(
  ...
  geom_text(
    ...
    , aes(label = Freq), vjust=1.6, color = "black", size = 6
    ) +
  annotate(
    "label"
    , x = 3.2
    , y = 70
    , label = "bold(n) == ~360/361"
    , parse = TRUE
    ) + 
  theme( 
    axis.text.y = element_text(size=g_tick_size_side)
...
```{r D3-bar, echo=FALSE, fig.width = 6, fig.height=4, dev='svg', fig.retina=2, fig.align='center'}
# D3: Was there an impulse from the non-academic world that motivated you 
# to start the project, namely ...
# <Code-Reviewed>

# -> Calculate also the "other" category
other <- ifelse(data$impulseForNonAcad.other. == "", "", "Y")

# -> Calc. frequencies under domains
soc_dom <- as.data.frame(table(data$domain, data$impulseForNonAcad.soc. == "Y")) %>%
  filter(Var2 == TRUE) %>%
  select(-Var2)
eco_dom <- as.data.frame(table(data$domain, data$impulseForNonAcad.econ. == "Y")) %>%
  filter(Var2 == TRUE) %>%
  select(-Var2)
nat_dom <- as.data.frame(table(data$domain, data$impulseForNonAcad.ecol. == "Y")) %>%
  filter(Var2 == TRUE) %>%
  select(-Var2)
hea_dom <- as.data.frame(table(data$domain, data$impulseForNonAcad.health. == "Y")) %>%
  filter(Var2 == TRUE) %>%
  select(-Var2)
tec_dom <- as.data.frame(table(data$domain, data$impulseForNonAcad.tech. == "Y")) %>%
  filter(Var2 == TRUE) %>%
  select(-Var2)
oth_dom <- as.data.frame(table(data$domain, other == "Y")) %>%
  filter(Var2 == TRUE) %>%
  select(-Var2)

# -> Collect under the same df
d3.freq_df <- rbind(
  soc_dom
  , eco_dom
  , nat_dom
...
d3.freq_df$Category <- rep(
  cats
  , each = 3
)
d3.freq_df$Category <- factor(d3.freq_df$Category,
       levels = cats)


# -> Visualise
g <- ggplot(d3.freq_df
            , aes(fill = Var1 ,x = Category, y = Freq)
            )
...
#### How many of the projects were not transdisciplinary?
```{r, echo=FALSE, fig.width=5, fig.height=3, dev='svg', fig.retina=2, fig.align='center'}
e1.df.nona <- na.omit(e1.df)
e1.df.nona_trans <- e1.df.nona[, 2:ncol(e1.df.nona)]

trans_table <- table(rowSums(e1.df.nona_trans != 0))

trans_df <- data.frame(
  types = c("No trans. inv.", "3 or fewer types", "More than 3 types (max. 6)"),
  num = c(trans_table[1], sum(trans_table[2:4]), sum(trans_table[5:7]))
)

trans_df$types <- factor(trans_df$types, levels = trans_df$types)
g <- ggplot(trans_df, aes(x = types, y=num))
g + geom_col(fill=col_bar) + 
  theme_light() + theme(axis.text.x = element_text()) + 
  scale_x_discrete("Number of transdisciplinary involvement types") + 
  scale_y_continuous("count") + 
  geom_text(aes(label=paste0(num, "\n", round(num/361, 2)* 100, "%")), vjust=1.6, color="white", size=5) + theme( axis.text.y = element_text( 
    size=g_tick_size_side)
    , axis.text.x = element_text(
      size=g_tick_size_side)
    , axis.title=element_text(size=g_tick_size_side)
    , legend.text=element_text(size=g_tick_size_side)) + 
  annotate("label", x = 3.2, y = 140, label = "bold(n) == ~342/361", parse = TRUE) +
  labs(caption = paste0("\nTransdisciplinary involvement types:\n"
                        , "* company/business representatives"
                        , "\n* representatives of NGOs, civil society groups", "\n* policy makers, public administrations etc."
                        , "\n* individual citizens"
                        , "\n* media representatives"
                        , "\n* representatives from welfare- or education-providing institutions")) 
```
...
```{r E3_bar, echo=FALSE, fig.width=6, dev='svg', fig.retina=2, fig.align='center'}
# Please indicate whether the following statements 
# fit your project:   
# Your project has ... 
# <code-reviewed>
e3.freq_df <- as.data.frame(
  sapply(FUN = table, e3.df)
  )

# Some of the variables are pointing to concrete results other intended
# -> Split e3 into 2 main groups: concrete, intended
e3_concrete.freq_df <- e3.freq_df %>%
  select(c("targetGroupsGoals.diversity.", "targetGroupsGoals.socgroups."))
e3_intended.freq_df <- e3.freq_df %>%
  select(-c("targetGroupsGoals.diversity.", "targetGroupsGoals.socgroups."))

# -> Adjust for ggplot type 1 freq column design 
e3_concrete.freq_df.trans <- transpose(e3_concrete.freq_df)
e3_concrete.freq_df.trans$statement <- colnames(e3_concrete.freq_df)
colnames(e3_concrete.freq_df.trans) <- c("No", "Yes")
e3_concrete.freq_df.trans <- e3_concrete.freq_df.trans %>%
  gather("Response", "Freq", 1:2)
e3_concrete.freq_df.trans$Freq <- as.integer(e3_concrete.freq_df.trans$Freq)
colnames(e3_concrete.freq_df.trans)[1] <- "Statement"

e3_intended.freq_df.trans <- transpose(e3_intended.freq_df)
e3_intended.freq_df.trans$statement <- colnames(e3_intended.freq_df)
colnames(e3_intended.freq_df.trans) <- c("No", "Yes")
e3_intended.freq_df.trans <- e3_intended.freq_df.trans %>%
  gather("Response", "Freq", 1:2)
e3_intended.freq_df.trans$Freq <- as.integer(e3_intended.freq_df.trans$Freq)
colnames(e3_intended.freq_df.trans)[1] <- "Statement"

# -> Visualise "concrete"
g_concrete <- ggplot(e3_concrete.freq_df.trans
            , aes(fill = Response, x = Statement, y = Freq)
            )
g_concrete <- g_concrete +
  ...
  geom_text(
    position = position_dodge(width= 0.9)
    , aes(label = Freq), hjust=1.6, color = "white", size = 6
    ) +
  theme( 
    axis.text.y = element_text(size = g_tick_size_side)
    , axis.text.x = element_text(size = g_tick_size_side)
  ...
  scale_fill_manual(
    values = c(col_bar2, col_bar)
    , name = "Response"
    ) +
  scale_x_discrete(
    labels = c("targetGroupsGoals.diversity." = "...enabled diversity and exchange"
               ,"targetGroupsGoals.socgroups." = "...included soc. disadvantaged or marginalised people."
               )
    ) + 
  coord_flip()
...
# -> Visualise "concrete"
g_intended <- ggplot(e3_intended.freq_df.trans
            , aes(fill = Response, x = Statement, y = Freq)
            )
g_intended <- g_intended +
  ...
  scale_x_discrete(
    labels = c("targetGroupsGoals.socneeds." = "...targeted a group of people with spec. social needs."
               ,"targetGroupsGoals.improve." = "...worked towards improving people’s lives."
               ,"targetGroupsGoals.empower." = "...aimed at empowering people"
               )
    ) + 
  coord_flip()
...
###### Gender Dimension among sci fields
```{r}
gender <-   ifelse(data$concepts2== "Y", 1, 0)

gender_con_df <- data.frame(gender_con = data$concepts2
                            , domain = data$domain
)
...
```{r G2_bar, echo=FALSE, fig.width=8, fig.height=3, dev='svg', fig.retina=2, fig.align='center'}
...
ggplot(den3_df,
  ...
  labs(
    #title = "Effect of Vitamin C on Tooth Growth",
    title = "What kind of change (short- or long-term) did your project\nintend to bring about in the following target groups or in the general population?") +  
...
a <- nat_change[, c(1,5)] %>% 
  ...
  gather(key=`Citizens (NoI)`, value=`Citizens (KoC)`) %>%
  ggplot(aes(x=`Citizens (NoI)`, y=`Citizens (KoC)`, fill=`Citizens (KoC)`),  show.legend = FALSE) +
  geom_boxplot() +
  ylab(" ") +
  theme_light() +
  theme(axis.text.y = element_blank(), legend.position = "none") +
...
```
```{r, echo=FALSE, fig.width=9, fig.height=6, dev='svg', fig.retina=2, fig.align='center'}
library(GGally)
#ggcorr(nat_change, method = c("pairwise", "spearman")) 
?ggcorr

ggpairs(nat_change, lower = list( combo = "box_no_facet")) + 
  theme_light()
...
g <- ggplot(g4_freq.df, aes(x=g4.df, y=Freq))
# Number of cars in each class:
g + geom_col(fill="#338BBE")+
  #scale_x_discrete("", labels = c("No", "Yes")) +
  scale_y_continuous("count") + 
...
###### Correlation between the intent and uptake policy making

```{r, echo=FALSE, fig.width=3, dev='svg', fig.retina=2, fig.align='center'}
policy_intent <- ifelse(data$concepts3 == "Y", 1, 0)
policy_uptake <- g4.df

policy_df <- data.frame(
  policy_intent = data$concepts3,
  policy_uptake = g4.df
)
colnames(policy_df)[2] <- "policy_uptake"
unique(data$concepts3)
policy_df <- policy_df[!(data$concepts3 == ""), ]

policy_df$policy_intent <- ifelse(policy_df$policy_intent == "Y", "Yes", "No")
policy_df %>% 
  filter(!is.na(policy_intent)) %>%
  filter(!is.na(policy_uptake)) %>%
  gather(key="policy_uptake", value="policy_intent") %>%
  ggplot(aes(x=policy_uptake, y=policy_intent, fill=policy_intent),  show.legend = FALSE) +
  geom_boxplot() +
  ylab(" ") +
  theme_light() +
  theme(axis.text.y = element_blank()) +
  scale_x_continuous(breaks = seq(0,10,2)) +
  xlab("Uptake of the project results by policymakers") + 
  ylab(" ") + 
  scale_fill_manual(values=col_seq[c(1,2)], name = "Intent to support policymaking")+guides(fill = guide_legend(reverse = TRUE)) + annotate("label", x = 9, y = 2.4, label = "bold(n) == 356/361", parse = TRUE)

library(papaja)
policy_df$policy_intent <- ifelse(policy_df$policy_intent == "Yes", 1, 0)
a <- psych::corr.test(policy_df, method = "spearman", use = "pairwise.complete.obs", n=T )
printnum(a, gt1 = FALSE, digits = 2)
...
```{r G5_bar, echo=FALSE, fig.width=4, dev='svg', fig.retina=2, fig.align='center'}
...
g + geom_col(fill="#D32F49")+
  ...
  annotate("label", x = 3.2, y = 29, label = "bold(n) == 62/361", parse = TRUE) 

den <- unlist(as.numeric(as.vector(feat_df$adoptByPolicyHow.SQ003. )) )+ as.numeric(as.vector(feat_df$adoptByPolicyHow.SQ002. ))+ as.numeric(as.vector(feat_df$adoptByPolicyHow.SQ001. ))
sum(den>0)
...
```{r H1_bar, echo=FALSE, fig.width=6, fig.height=4, dev='svg', fig.retina=2, fig.align='center'}
# Dissemination Channels under trans inv.
...
trans_bool <- rowSums(e1.df.nona_trans != 0) 
trans_bool <- ifelse(trans_bool == 0, 0, 1)
h12.df$trans <- trans_bool

h12.freq_df <- as.data.frame(h12.df %>%
  sapply(FUN= table, trans_bool))

h12.freq_df$trans <- c("NoTrans-NoDiss", "NoTrans-YesDiss", "YesTrans-NoDiss", "YesTrans-YesDiss")

no_trans <- h12.freq_df[1:2, ]
yes_trans <- h12.freq_df[3:4, ]
trans_df <-  h12.freq_df[c(2,4), ]



no_trans <- transpose(no_trans[, 1:(ncol(no_trans) - 1)] )
colnames(no_trans) <- c("No", "Yes")
no_trans.nrow <- nrow(no_trans)
no_trans <- no_trans %>%
 gather("Dissemination" ,"Freq", 1:2)
no_trans$Diss_name <- rep(colnames(h12.freq_df)[1:(ncol(h12.freq_df) - 1)], 2)


yes_trans <- transpose(yes_trans[, 1:(ncol(yes_trans) - 1)] )
colnames(yes_trans) <- c("No", "Yes")
yes_trans.nrow <- nrow(yes_trans)
yes_trans <- yes_trans %>%
 gather("Dissemination" ,"Freq", 1:2)
yes_trans$Diss_name <- rep(colnames(h12.freq_df)[1:(ncol(h12.freq_df) - 1)], 2)

trans_df <- transpose(trans_df[, 1:(ncol(trans_df) - 1)] )
colnames(trans_df) <- c("No", "Yes")
trans_df.nrow <- nrow(trans_df)
trans_df <- trans_df %>%
 gather("Transdisciplinarity" ,"Freq", 1:2)
trans_df$Diss_name <- rep(colnames(h12.freq_df)[1:(ncol(h12.freq_df) - 1)], 2)

# -> Visualise "concrete"
g_trans <- ggplot(trans_df[-c(1,2, 10, 11),]
            , aes(fill = Transdisciplinarity, x = Diss_name , y = Freq)
            )
g_trans <- g_trans +
  geom_bar(position = "dodge", stat = "identity") +
  theme_light() + 
  ...
  scale_fill_manual(
    values = c(col_bar2, col_bar)
    , name = "Transdisciplinary involvement"
    ) + 
  coord_flip() +
  scale_x_discrete(labels=c( #"dissChannels.peer." = "Peer reviewed journal publication"
                            ...
                            , "dissChannels.prof." = "Professional journals/magazines"
                            , "dissChannels.policy." = "Policy brief(s)"
                            , "dissChannels.web." = "Own institutional or project website/blog"
                            , "dissChannels.socmed." = "Social media"
                            , "dissChannels.platf." = "Online platforms"
                            , "dissChannels.events." = "Targeted events for practitioners"
                            )
                   ) + xlab("Dissemination type")
...
```{r H2_bar, echo=FALSE, fig.width=6, fig.height=2, fig.retina=2, dev = "svg", fig.align='center'}

h2.df_named <- h2.df[, 1:2]
names(h2.df_named) <- c(
...
outcome_likert + theme( axis.text.y = element_text( 
  size=g_tick_size_side), axis.text.x = element_text(
    size=g_tick_size_side), axis.title=element_text(size=g_tick_size_side), legend.text=element_text(size=g_tick_size_side)) +
  annotate("label", x = 1, y = 100, label = "bold(n) == 149/361", parse = TRUE) +
  annotate("label", x = 2, y = 100, label = "bold(n) == 172/361", parse = TRUE) 
...
```{r, echo=FALSE, fig.width=6, fig.retina=2, dev = "svg", fig.align='center'}
sample_df <- read.csv("./01_data/snf_df_-_fname_lname.csv")

sample_funding <- as.data.frame(table(sample_df$SP.Funding) )
respondent_funding <- as.data.frame(table(data$funding))

# Compute percentages
sample_funding$fraction <- sample_funding$Freq / sum(sample_funding$Freq)
respondent_funding$fraction <- respondent_funding$Freq / sum(respondent_funding$Freq)

# Compute the cumulative percentages (top of each rectangle)
sample_funding$ymax <- cumsum(sample_funding$fraction)
respondent_funding$ymax <- cumsum(respondent_funding$fraction)

# Compute the bottom of each rectangle
sample_funding$ymin <- c(0, head(sample_funding$ymax, n=-1))
respondent_funding$ymin <- c(0, head(respondent_funding$ymax, n=-1))

# Compute label position
sample_funding$labelPosition <- (sample_funding$ymax + sample_funding$ymin) / 2
respondent_funding$labelPosition <- (respondent_funding$ymax + respondent_funding$ymin) / 2

# Compute a good label
sample_funding$label <- paste0(sample_funding$Freq, "\n", round(sample_funding$fraction, 2) * 100, "%")
respondent_funding$label <- paste0(respondent_funding$Freq, "\n", round(respondent_funding$fraction, 2) * 100, "%")

# Make the plot
a <- ggplot(sample_funding, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Var1)) +
  geom_rect() +
  geom_label( x=3.5, aes(y=labelPosition, label=label), size=6) +
  scale_fill_brewer(palette="Pastel2") +
  coord_polar(theta="y") +
  xlim(c(2, 4)) +
  theme_void() + guides(fill = guide_legend(
...
a#+
#  theme(legend.position = "none")
b <- ggplot(respondent_funding, aes(ymax=ymax, ymin=ymin, xmax=4, xmin=3, fill=Var1)) +
  geom_rect() +
  geom_label( x=3.5, aes(y=labelPosition, label=label), size=6) +
  scale_fill_brewer(palette="Pastel2") +
  coord_polar(theta="y") +
...
samres_funding <- as.data.frame(cbind(
  programme = rep(as.character(sample_funding[, 1]), 2)
  , Freq = as.numeric(c(sample_funding$Freq, respondent_funding$Freq ))
  , Category = c(rep("Sample",4), rep("Respondents", 4)))
)
samres_funding$Freq <- as.numeric(samres_funding$Freq)
class(samres_funding$Freq)
ggplot(samres_funding, aes(fill=Category, y=Freq, x=programme)) + 
  geom_bar(position="dodge", stat="identity") +
  xlab("Funding programmes") +
...
```{r echo=FALSE, fig.width=6 , fig.retina=2, fig.align='center', dev='svg'}
d1.df_named <- d1.df
# names(d1.df_named) <- c(
#   motivation.pheno. = "understand a natural, tech., econ., or social phenomenon?",
#   motivation.prob. = "address a natural, tech., econ., or social problem?",
...
d1.df_named$funding_programme <- data$funding

d2domain.df$funding_programme <- data$funding
benefit.df <- data.frame(benefit = data$benefitForNonAcademy,
                         funding_programme = data$funding)
...
d1.df_named$motivation.pheno. <- whatever(d1.df_named$motivation.pheno.)
d1.df_named$motivation.prob. <- whatever(d1.df_named$motivation.prob.)
d1.df_named$motivation.welfare. <- whatever(d1.df_named$motivation.welfare.)

pheno_prog <- as.data.frame(table(d1.df_named[,c(1,4)]))
pheno_prob <- as.data.frame(table(d1.df_named[,c(2,4)]))
pheno_welf <- as.data.frame(table(d1.df_named[,c(3,4)]))
benef_prog <- as.data.frame(table(benefit.df))


a <- ggplot(pheno_prog, aes(fill=funding_programme, y=Freq, x=motivation.pheno.)) + 
  geom_bar(position="dodge", stat="identity") +
  xlab("Motivation to better understand a natural, technical, economic, or social phenomenon") +
  ylab("Count") + scale_x_discrete(limits= c("0", "1", "2"), labels = c("0...not at all - 3", "4 - 6", "7 - 10...fully")) + 
  theme_light()+ geom_text(position = position_dodge(width= 0.9), aes(y=Freq+0.25, fill=funding_programme, label=Freq, hjust=0.3, vjust = -0.9)) +theme(axis.text.x = element_text( hjust = 1))  +scale_fill_brewer(palette="Pastel2", name="Funding instrument") + theme(legend.position = "none")+  annotate("label", x = 0.6, y = 120, label = paste0("bold(n) ==", sum(pheno_prog$Freq),"/361"), parse = TRUE)

b <- ggplot(pheno_prob, aes(fill=funding_programme, y=Freq, x=motivation.prob.)) + 
  geom_bar(position="dodge", stat="identity") +
  xlab("Motivation to directly address a natural, technical, economic, or social problem?") +
  ylab("Count") + scale_x_discrete(limits= c("0", "1", "2"), labels = c("0...not at all - 3", "4 - 6", "7 - 10...fully")) + 
  theme_light()+ geom_text(position = position_dodge(width= 0.9), aes(y=Freq+0.25, fill=funding_programme, label=Freq, hjust=0.3, vjust = -0.9)) +theme(axis.text.x = element_text( hjust = 1))  +scale_fill_brewer(palette="Pastel2", name="Funding instrument")  + theme(legend.position = "none") +  annotate("label", x = 0.6, y = 120, label = paste0("bold(n) ==", sum(pheno_prob$Freq),"/361"), parse = TRUE)

c <- ggplot(pheno_welf, aes(fill=funding_programme, y=Freq, x=motivation.welfare.)) + 
  geom_bar(position="dodge", stat="identity") +
  xlab("Motivation to improve the human condition/welfare (outside academia)?
") +
  ylab("Count") + scale_x_discrete(limits= c("0", "1", "2"), labels = c("0...not at all - 3", "4 - 6", "7 - 10...fully")) + 
  theme_light()+ geom_text(position = position_dodge(width= 0.9), aes(y=Freq+0.25, fill=funding_programme, label=Freq, hjust=0.3, vjust = -0.9)) +theme(axis.text.x = element_text( hjust = 1))  +scale_fill_brewer(palette="Pastel2", name="Funding instrument") + 
  annotate("label", x = 0.6, y = 120, label = paste0("bold(n) ==", sum(pheno_welf$Freq),"/361"), parse = TRUE)
a + b + c




benef_plot <- ggplot(benef_prog, aes(fill=funding_programme, y=Freq, x=benefit)) + 
  geom_bar(position="dodge", stat="identity") +
  xlab("Have you deliberately designed your project so that it might generate an immediate and intended benefit for the general population or a specific non-academic target group?") +
  ylab("Count") + scale_x_discrete(limits= c("1", "2", "3"), labels = c("1" = "no", "2"="to a minor extent", "3"="to a large extent")) + 
  theme_light()+ geom_text(position = position_dodge(width= 0.9), aes(y=Freq+0.25, fill=funding_programme, label=Freq, hjust=0.3, vjust = -0.9)) +theme(axis.text.x = element_text( hjust = 1))  +scale_fill_brewer(palette="Pastel2", name="Funding instrument") + 
  annotate("label", x = 3.3, y = 110, label = paste0("bold(n) ==", sum(benef_prog$Freq),"/361"), parse = TRUE)
